...
class VisitIdPmIdImagesIdController extends GetxController {
  final db = XanX.db();
  final visitId = int.parse(Get.parameters['id']!);
  final pmId = int.parse(Get.parameters['id2']!);
  final imgId = int.parse(Get.parameters['id3']!);

  final rxImg = Rxn<BroFa2VisitPmImg>();

  @override
  void onInit() async {
    super.onInit();
    rxImg.value = await db.broFa2VisitPmImgDao.getById(imgId);
  }
  ...
  void deleteImg() {
    ...
    XanX.showConfirmDialog(
      ...
      vcb: () async {
        ...
        await db.broFa2VisitPmImgDao.deleteByPk(imgId);
        Get.back();
      },
    );
  }
